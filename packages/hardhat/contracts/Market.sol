...
contract DisasterManagement {
    ...
    function getDisasterReport(uint256 _index)
        public
        view
        returns (
            address,
    ...
    function deleteDisasterReport(uint256 id) public {
        ...
        require(msg.sender == disasterReports[id].reporterId, "Please ensure you are the owner of this request");
    ...
    function addDisasterImage(uint256 id, string memory _imageUrl, string memory _timestamp) public {
        require(id < disasterReportLength, "Invalid report index");
        require(msg.sender == disasterReports[id].reporterId, "You are not the owner of this report");
        disasterImages[id].push(
            DisasterImage({reporterId: msg.sender, disasterImageUrl: _imageUrl, timestamp: _timestamp})
        );
    }
    ...
    function deleteDisasterImage(uint256 id, uint256 imageIndex) public {
        ...
        disasterImages[id][imageIndex] = disasterImages[id][disasterImages[id].length - 1];
